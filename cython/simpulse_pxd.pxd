...
cdef extern from "simpulse_cython.hpp" namespace "simpulse":
    ...
    cdef cppclass single_pulse:
        ...
        double get_signal_to_noise(double sample_dt, double sample_t0, double sample_rms)
        string str()

    void _add_to_timestream_float(single_pulse *sp, float *out, double out_t0, double out_t1, int out_nt, int stride)
    void _add_to_timestream_double(single_pulse *sp, double *out, double out_t0, double out_t1, int out_nt, int stride)
