...
cdef class single_pulse:
    ...
    def _add_to_timestream_float(self, np.ndarray[float,ndim=2,mode='c'] out not None, out_t0, out_t1):
        simpulse_pxd._add_to_timestream_float(self.p, <float *> &out[0,0], out_t0, out_t1, out.shape[1], out.shape[1])

    def _add_to_timestream_double(self, np.ndarray[double,ndim=2,mode='c'] out not None, out_t0, out_t1):
        simpulse_pxd._add_to_timestream_double(self.p, <double *> &out[0,0], out_t0, out_t1, out.shape[1], out.shape[1])

    def add_to_timestream(self, np.ndarray out not None, out_t0, out_t1):
        """
        ...
        if out.dtype == np.float32:
            self._add_to_timestream_float(out, out_t0, out_t1)
        elif out.dtype == np.float64:
            self._add_to_timestream_double(out, out_t0, out_t1)
        else:
